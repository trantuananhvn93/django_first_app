...
class OrderSummaryView(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        try:
            ...
            return render(self.request, 'polls/order_summary.html', context)
...
def add_to_cart(request, item_id):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item_id=item.id).exists():
            ...
            order.save()  
            messages.info(request, "This item is succesfully added to your cart")
    else:
        order = Order.objects.create(user = request.user, order_date = timezone.now)
        order.items.add(order_item)
        order.save()
        messages.info(request, "This item is succesfully added to your cart, your cart is not empty now")
...
def remove_single_item_from_cart(request, item_id):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item_id=item.id).exists():
            ...
            return HttpResponseRedirect(reverse("polls:order-summary"))
        else:
            messages.info(request, "This item was not in your cart")
            return HttpResponseRedirect(reverse('polls:product', args=(item.id,)))
    else:
...
class RequestRefundView(View):
    def get(self, *args, **kwargs):
        ...
        return render(self.request, "polls/request_refund.html", context)
